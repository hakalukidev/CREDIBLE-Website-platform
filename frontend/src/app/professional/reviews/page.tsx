...
export default function ProfessionalReviewsPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['reviews', 'professional', 'owner', page, perPage, filters],
    queryFn: async () => {
      ...
      try {
        ...
        }>(`/professionals/me/reviews?${params.toString()}`);
        return res.data;
      } catch (err) {
        // Fall back to empty list if the endpoint isn't wired yet — so the
        // page doesn't crash during incremental rollout.
        if ((err as { response?: { status?: number } })?.response?.status === 404) {
          return { success: true as const, data: [], meta: { totalPages: 1 } };
        }
        throw err;
      }
    },
    ...
    <div className="space-y-4">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Respond to your clients and report anything that doesn't meet our community guidelines.
        </p>
      </header>
