...
/**
 * ChoiceStep — first step of the public Register wizard.
 *
 * Lives on `/profile/register` (under the profile chrome) so visitors
 * who arrive from the "Register a Business" tab land in a public page
 * without any dashboard shell. The two real forms (Business /
 * Professional) already handle draft / edit / category / image-upload /
 * hours, so we never reimplement them — this wizard just chooses
 * which form to mount next.
 *
 * No dashboard imports. The choice step renders standalone and the
 * wizard wires the URL state for step 2.
 */
