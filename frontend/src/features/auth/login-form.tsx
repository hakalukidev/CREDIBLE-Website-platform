...
export function LoginForm({
  ...
  const login = useMutation({
    ...
    onSuccess: (data) => {
      ...
      router.push(postAuthRedirect(search, data.user.role, { adminOnly }) as any);
    },
  ...
  async function handleOAuth(provider: 'google' | 'facebook') {
    ...
    try {
      ...
      router.push(postAuthRedirect(search, session.user.role, { adminOnly }) as any);
    } catch (err) {
      ...
      >
        ...
        {!adminOnly && (providers.google || providers.facebook) && (
          <div className={variant === 'google' ? 'mt-6 space-y-3' : 'mt-6 space-y-6'}>
            ...
            <div className={variant === 'google' ? 'flex flex-col gap-2' : 'grid grid-cols-2 gap-3'}>
              {providers.google && (
                <Button
                  type="button"
