...
import { MotionCardStagger, MotionCardReveal } from '@/components/ui/motion-primitives';
...
export function FeaturedBusinesses({
  ...
  return (
    <MotionCardStagger className="grid grid-cols-1 items-stretch gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {items.map((b) => (
        <MotionCardReveal key={b.id} className="h-full">
          <BusinessCard
            id={b.id}
            slug={b.slug}
            name={b.displayName}
