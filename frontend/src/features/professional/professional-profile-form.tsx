...
function CreateProfessionalForm() {
  ...
  const create = useMutation({
    mutationFn: async (values: CreateProfessionalInput) => {
      // Backend returns `{ professional, user }` so the frontend can
      // refresh the session after the role upgrade.
      ...
      // The create transaction upgraded the user CUSTOMER → PROFESSIONAL
      // server-side, but the JWT still carries the old role until
      // it's refreshed. Swap tokens first so the publish call (gated
      // on `requireRole('PROFESSIONAL')`) and the form's re-fetch of
      // `/professionals/me/profile` don't 403.
      await refreshSessionTokens();

      // Auto-publish so the new professional profile shows up on
      // /professionals/search (which filters on `status: 'PUBLISHED'`).
      // Best-effort: a failure here doesn't reject the whole create
      // mutation — the user can retry from their dashboard.
      try {
    ...
    onSuccess: ({ user }) => {
      toast.success('Professional profile created and published!');
