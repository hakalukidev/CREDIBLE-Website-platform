...
/**
 * Re-usable popup dialog that hosts the subscription manager over the
 * dashboard. The same body is also rendered as a plain page at
 * `/business/subscription` so direct links and refreshes still work.
 */
export function SubscriptionDialog(props: Props) {
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className="max-h-[90vh] w-[min(96vw,820px)] max-w-none overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
        ...
        <CardContent>
          ...
          {!sub.cancelAtPeriodEnd && sub.plan !== 'FREE' && (
            <div className="mt-4 flex items-center justify-between rounded-md border p-3 text-sm">
              <span>
                Need to cancel? Your access continues until{' '}
                {formatDate(sub.currentPeriodEnd)}.
              </span>
              <CancelInline
                onCancel={(reason) => cancel.mutate({ immediate: false, reason })}
              />
            </div>
          )}
        </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <Button asChild variant="ghost" size="sm" className="w-full">
              <Link href="/business/subscription/invoices">
                View all invoices
              </Link>
            </Button>
          </CardContent>
...
function PaymentStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-xs font-medium ${variant}`}
    >
      {status}
    </span>
  );
