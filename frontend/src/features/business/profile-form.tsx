...
import { useCategories, type CategoryOption } from '@/features/categories/use-categories';
import {
  BUSINESS_DAYS,
  BUSINESS_DAY_LABELS,
...
function CreateBusinessForm() {
  ...
  const create = useMutation({
    mutationFn: async (values: CreateFormValues) => {
      // Backend now returns `{ business, user }` so the frontend can
      // refresh the session after the role upgrade. We only need the
      // id and the user payload here.
      ...
      // The create transaction upgraded the user CUSTOMER → BUSINESS
      // server-side, but the JWT used for subsequent requests still
      // carries the old role until it's refreshed. Swap tokens first
      // so the publish call (gated on `requireRole('BUSINESS')`) and
      // the form's re-fetch of `/businesses/me/profile` don't 403.
      // `refreshSessionTokens` swallows errors — we keep going either
      // way and let the `requireRole('CUSTOMER')`-aware route handle
      // it (defense in depth in routes/businesses/business.routes.ts).
      await refreshSessionTokens();

      // Auto-publish so the new business shows up on /browse (which
      // filters on `status: 'PUBLISHED'`). Best-effort: a failure here
      // doesn't reject the whole create mutation — the user can retry
      ...
      try {
        ...
        debugWarn('[register] auto-publish failed:', publishErr);
      }

      return created;
    },
    onSuccess: ({ user }) => {
      toast.success('Business created and published to Browse!');
