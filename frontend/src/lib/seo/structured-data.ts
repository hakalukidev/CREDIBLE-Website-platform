...
export const SITE_NAME = 'Credible';
...
/**
 * Blog index page schema. Lists each post as a `BlogPosting` so Google can
 * surface individual posts in the article carousel / "Latest from" block.
 * The `publisher` @id keeps every post linked to the canonical Organization
 * node declared in `organizationSchema()`.
 */
export function blogSchema(posts: Array<{ slug: string; title: string; excerpt: string; date: string; author: { name: string } }>) {
  return {
    '@context': 'https://schema.org',
    '@type': 'Blog',
