...
export const qk = {
  ...
  businesses: {
    ...
    /**
     * Prefix used for broad invalidations (e.g. after the owner
     * creates a new business). React Query treats this as a prefix
     * match, so it sweeps `me`, `profile`, and any other `businesses`
     * keys without touching `business-search` (the search lives in a
     * separate key namespace because it's a plain `apiClient.get`
     * in Browse, not a cached query).
     */
