...
export const reviewController = {
  ...
  async listForBusiness(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { items, total, distribution } = await reviewService.listForBusiness(
  ...
  async listForProfessional(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { minRating } = req.query as { minRating?: string };
      const { rating } = req.query as { rating?: string };
      const { items, total, distribution } = await reviewService.listForProfessional(
