...
export const professionalController = {
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      const { professional, user } = await professionalService.create(req.user!.id, req.body);
      // Service returns `{ professional, user }` so the frontend can
      // refresh its session store after the role upgrade without
      // hitting the DB again.
      res.status(201).json({ success: true, data: { professional, user } });
    } catch (e) {
      next(e);
    }
  },
